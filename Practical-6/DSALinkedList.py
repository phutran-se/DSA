...
class DSALinkedList:
    ...
    def get_items(self):
        ...
        length = 0          # Initialize a length counter
        ...
        # Create a numpy array with the correct length
        values = np.empty(length, dtype=object)
        
        # Traverse the list again and populate the numpy array
        curr = self.head
        index = 0
        while curr:
            values[index] = curr.value  # Add node value to numpy array
            curr = curr.next
            index += 1
        
        return values  # Return the numpy array of values               # Return the list of values
